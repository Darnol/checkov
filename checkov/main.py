...
from checkov.common.bridgecrew.bc_source import (
    SourceTypes,
    BCSourceType,
    get_source_type,
    SourceType,
...
from checkov.common.bridgecrew.platform_errors import (
...
from checkov.common.util.banner import (
    banner as checkov_banner,
    default_tool as default_tool,
)
...
class Checkov:
            ...
            for i, arg in enumerate(argv):
                if arg == "--config-file" and i + 1 < len(argv):
                    ...
                    if not config_path.is_file():
                        logger.debug(
                            f"The config file at '{config_path}' does not exist. Running without a config file."
                        )
    ...
    def normalize_config(self) -> None:
        if not self.config.bc_api_key and not self.config.include_all_checkov_policies:
            ...
            logger.debug(
                "No API key present; setting include_all_checkov_policies to True"
            )
        ...
        if (
            self.config.policy_metadata_filter
            or self.config.policy_metadata_filter_exception
        ) and not (self.config.bc_api_key and self.config.prisma_api_url):
            logger.warning(
        ...
        self.config.framework = self.normalize_framework_arg(
            self.config.framework, handle_all=True
        )
        logging.debug(f"Normalized --framework value: {self.config.framework}")

        logging.debug("Normalizing --skip-framework")
        self.config.skip_framework = self.normalize_framework_arg(
            self.config.skip_framework
        )
        logging.debug(
            f"Normalized --skip-framework value: {self.config.skip_framework}"
        )

        duplicate_frameworks = set(self.config.skip_framework).intersection(
            self.config.framework
        )
        if duplicate_frameworks:
            self.parser.error(
                f"Frameworks listed for both --framework and --skip-framework: {', '.join(duplicate_frameworks)}"
            )
    ...
    def normalize_framework_arg(
        self, raw_framework_arg: List[List[str]], handle_all: bool = False
    ) -> List[str]:
        ...
        if raw_framework_arg:
            ...
            if handle_all and "all" in frameworks:
                ...
                if invalid:
                    self.parser.error(
                        f"Invalid frameworks specified: {', '.join(invalid)}.{os.linesep}"
        ...
        try:
            ...
            if self.config.bc_api_key and not self.config.include_all_checkov_policies:
                if self.config.skip_download and not self.config.external_checks_dir:
                    ...
                    print(
                        "You are using an API key along with --skip-download but not --include-all-checkov-policies. "
                        "With these arguments, Checkov cannot fetch metadata to determine what is a local Checkov-only "
            ...
            bc_integration.setup_http_manager(
                self.config.ca_certificate, self.config.no_cert_verify
            )
            ...
            if not self.config.bc_api_key:
                ...
                severities = {
                    severity
                    for severity in BcSeverities.__dict__.values()
                    if isinstance(severity, str)
                }
                if (
                    self.config.check
                    and any(check in severities for check in self.config.check)
                ) or (
                    self.config.skip_check
                    and any(check in severities for check in self.config.skip_check)
                ):
                    logging.warning(
                        "Filtering checks by severity is only possible with an API key"
                    )

            excluded_paths = self.config.skip_path or []

            if self.config.var_file:
                self.config.var_file = [
                    os.path.abspath(f) for f in self.config.var_file
                ]
            ...
            runner_filter = RunnerFilter(
                ...
                download_external_modules=bool(
                    convert_str_to_bool(self.config.download_external_modules)
                ),
                external_modules_download_path=self.config.external_modules_download_path,
                evaluate_variables=bool(
                    convert_str_to_bool(self.config.evaluate_variables)
                ),
                ...
                enable_secret_scan_all_files=bool(
                    convert_str_to_bool(self.config.enable_secret_scan_all_files)
                ),
                ...
                report_sast_imports=bool(
                    convert_str_to_bool(
                        os.getenv("CKV_ENABLE_UPLOAD_SAST_IMPORTS", False)
                    )
                ),
                report_sast_reachability=bool(
                    convert_str_to_bool(
                        os.getenv("CKV_ENABLE_UPLOAD_SAST_REACHABILITY", False)
                    )
                ),
            )
            ...
            if outer_registry:
                ...
                runner_registry = RunnerRegistry(
                    banner, runner_filter, *self.runners, tool=tool
                )
            ...
            if self.config.bc_api_key == "":
                ...
                try:
                    ...
                    if bc_integration.prisma_api_url:
                        message = (
                            "An error occurred setting up the Prisma Cloud platform integration. "
                            "Please check your Prisma Cloud API token and URL and try again."
                        )
                    else:
                        message = (
                            "An error occurred setting up the Bridgecrew platform integration. "
                ...
                if bc_integration.support_flag_enabled:
                    logger.warning(
                        "--bc-api-key argument is required when using --support"
                    )
            ...
            if bc_integration.on_prem:
                ...
                if bc_integration.support_flag_enabled:
                    logger.warning(
                        "--support flag is not supported for on-premises integrations"
                    )
                ...
                if not outer_registry:
                    ...
                    if removed_check_types:
                        logger.warning(
                            f"Following runners won't run as they are not supported for on-premises integrations: {removed_check_types}"
                        )

            bc_integration.get_prisma_build_policies(
                self.config.policy_metadata_filter,
                self.config.policy_metadata_filter_exception,
            )
            ...
            policy_level_suppression = (
                suppressions_integration.get_policy_level_suppressions()
            )
            runner_filter.bc_cloned_checks = (
                custom_policies_integration.bc_cloned_checks
            )
            custom_policies_integration.policy_level_suppression = list(
                policy_level_suppression.keys()
            )
            runner_filter.set_suppressed_policies(
                list(policy_level_suppression.values())
            )

            if any(
                framework in runner_filter.framework
                for framework in ("all", CheckType.SCA_IMAGE)
            ):
                # only run image referencer, when sca_image framework is enabled
                runner_filter.run_image_referencer = (
                    licensing_integration.should_run_image_referencer()
                )

            runner_filter.filtered_policy_ids = (
                policy_metadata_integration.filtered_policy_ids
            )
            runner_filter.filtered_exception_policy_ids = (
                policy_metadata_integration.filtered_exception_policy_ids
            )
            logger.debug(
                f"Filtered list of policies: {runner_filter.filtered_policy_ids}"
            )
            logger.debug(
                f"Filtered excluded list of policies: {runner_filter.filtered_exception_policy_ids}"
            )

            runner_filter.excluded_paths = runner_filter.excluded_paths + list(
                repo_config_integration.skip_paths
            )

            if self.config.use_enforcement_rules:
                runner_filter.apply_enforcement_rules(
                    repo_config_integration.code_category_configs
                )

            if self.config.list:
                print_checks(
                    frameworks=self.config.framework,
                    use_bc_ids=self.config.output_bc_ids,
                    include_all_checkov_policies=self.config.include_all_checkov_policies,
            ...
            default_github_dir_path = (
                os.getcwd() + "/" + os.getenv("CKV_GITHUB_CONF_DIR_NAME", "github_conf")
            )
            git_configuration_folders = [
                os.getenv("CKV_GITHUB_CONF_DIR_PATH", default_github_dir_path),
                os.getcwd()
                + "/"
                + os.getenv("CKV_GITLAB_CONF_DIR_NAME", "gitlab_conf"),
            ]
            ...
            if self.config.directory:
                ...
                for root_folder in self.config.directory:
                    ...
                    if not os.path.exists(root_folder):
                        logger.error(
                            f"Directory {root_folder} does not exist; skipping it"
                        )
                        continue
                    ...
                    self.resource_subgraph_maps = (
                        runner_registry.check_type_to_resource_subgraph_map
                    )
                    ...
                    if (
                        bc_integration.is_integration_configured()
                ...
                should_run_contributor_metrics = (
                    ...
                    and self.config.repo_id
                    and self.config.prisma_api_url
                )
                logger.debug(
                    f"Should run contributor metrics report: {should_run_contributor_metrics}"
                )
                if should_run_contributor_metrics:
                    try:  # collect contributor info and upload
                        report_contributor_metrics(
                            self.config.repo_id, source.name, bc_integration
                        )
                    except Exception as e:
                        logger.warning(
                            f"Unable to report contributor metrics due to: {e}"
                        )
                ...
                if len(self.scan_reports) > 1:
                    ...
                    logger.error(
                        f"SCA image runner returned {len(self.scan_reports)} reports; expected 1"
                    )

                integration_feature_registry.run_post_runner(self.scan_reports[0])

                if (
                    not self.config.skip_results_upload
                    and not bc_integration.s3_setup_failed
                ):
                    try:
                        if not bc_integration.on_prem:
                            bc_integration.persist_repository(
                                os.path.dirname(self.config.dockerfile_path),
                                files=files,
                                sast_languages=runner_filter.sast_languages,
                            )
                        ...
                        bc_integration.persist_image_scan_results(
                            sca_runner.raw_report,
                        ...
                        logging.error(
                            "An error occurred while uploading scan results to the platform",
                            exc_info=True,
                        )
                ...
                exit_code = self.print_results(
                    runner_registry=runner_registry, url=self.url
                )
                return exit_code
                ...
                if self.config.create_baseline:
                    ...
                    for report in self.scan_reports:
                        overall_baseline.add_findings_from_report(report)
                    created_baseline_path = os.path.join(
                        os.path.abspath(os.path.commonprefix(self.config.file)),
                        ".checkov.baseline",
                    )
                    with open(created_baseline_path, "w") as f:
                        json.dump(overall_baseline.to_dict(), f, indent=4)
            ...
            if all(
                framework in self.config.framework
                for framework in m.unsupported_frameworks
            ):
                logging.warning(m)
            return None
        except PlatformConnectionError:
            ...
            if bc_integration.support_flag_enabled:
                ...
                if bc_integration.s3_setup_failed:
                    print_to_stderr = (
                        os.getenv("CKV_STDERR_DEBUG", "FALSE").upper() == "TRUE"
                    )
                    log_level = os.getenv("LOG_LEVEL", "")
                    if log_level == "DEBUG":
                        print(
                            "Unable to upload support logs. However, LOG_LEVEL is already set to DEBUG, so debug logs are available locally."
                        )
                    elif print_to_stderr:
                        print(
                            "Unable to upload support logs - CKV_STDERR_DEBUG is TRUE, printing to stderr."
                        )
                        ...
                        print(
                            "Unable to upload support logs - saving debug logs to ./checkov_debug.log. To print the debug "
                    ...
                    bc_integration.persist_all_logs_streams(
                        logger_streams.get_streams()
                    )
        ...
        try:
            ...
            if not bc_integration.on_prem:
                ...
                if git_configuration_folders:
                    bc_integration.persist_git_configuration(
                        os.getcwd(), git_configuration_folders
                    )
                if sca_supported_ir_report:
                    scan_reports_to_upload = [
                        report
                        for report in self.scan_reports
            ...
            bc_integration.persist_reachability_scan_results(
                self.sast_data.reachability_report
            )
            bc_integration.persist_run_metadata(self.run_metadata)
            if bc_integration.enable_persist_graphs and not bc_integration.on_prem:
                bc_integration.persist_graphs(
                    self.graphs, absolute_root_folder=absolute_root_folder
                )
    ...
    def save_sast_assets_data(self, scan_reports: List[Report]) -> None:
        if not bool(
            convert_str_to_bool(os.getenv("CKV_ENABLE_UPLOAD_SAST_IMPORTS", False))
        ):
            return
        sast_report = [
            scan_report
            for scan_report in scan_reports
            if isinstance(scan_report, SastReport)
        ]
    ...
    def save_sast_reachability_data(self, scan_reports: List[Report]) -> None:
        if not bool(
            convert_str_to_bool(os.getenv("CKV_ENABLE_UPLOAD_SAST_REACHABILITY", False))
        ):
            return
        sast_report = [
            scan_report
        ...
        for rep in sast_report:
            if rep.sast_reachability:
                result[rep.language] = {
                    **result[rep.language],
                    **serialize_reachability_report(rep.sast_reachability),
                }
