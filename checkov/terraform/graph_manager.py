...
class TerraformGraphManager(
    GraphManager[TerraformLocalGraph, "dict[TFDefinitionKey, dict[str, Any]]"]
):
    ...
    def build_multi_graph_from_source_directory(
        ...
        list[
            tuple[TerraformLocalGraph, list[dict[TFDefinitionKey, dict[str, Any]]], str]
        ],
        dict[str, str],
    ]:
        ...
        for module, tf_definitions in modules_with_definitions:
            ...
            subgraph_path = subgraph_abs_path[
                subgraph_abs_path.rindex(source_dir) + len(source_dir) + 1 :
            ]
            graphs.append((local_graph, tf_definitions, subgraph_path))
            self.update_resource_subgraph_map(
                local_graph, subgraph_path, resource_subgraph_map, source_dir
            )
    ...
    def build_graph_from_definitions(
        ...
        module, _ = self.parser.parse_hcl_module_from_tf_definitions(
    ...
    def build_multi_graph_from_definitions(
        ...
        module, tf_definitions = self.parser.parse_hcl_module_from_tf_definitions(
            definitions, "", self.source
        )
        ...
        for source_path, dir_definitions in dirs_to_definitions.items():
            module, parsed_tf_definitions = (
                self.parser.parse_hcl_module_from_multi_tf_definitions(
                    dir_definitions, source_path, self.source
                )
            )
            local_graph = TerraformLocalGraph(module)
            local_graph.build_graph(render_variables=render_variables)
