...
from checkov.common.runners.base_runner import (
    filter_ignored_paths,
    IGNORE_HIDDEN_DIRECTORY_ENV,
)
...
from checkov.terraform.modules.module_utils import (
    safe_index,
...
class TFParser:
    ...
    def _init(
        self,
        directory: str,
        out_evaluations_context: Dict[TFDefinitionKey, Dict[str, EvaluationContext]]
        | None,
        ...
        self.out_definitions: dict[
            TFDefinitionKey, dict[str, list[dict[str, Any]]]
        ] = {}
        self.out_evaluations_context = (
            {} if out_evaluations_context is None else out_evaluations_context
        )
        self.out_parsing_errors = (
            {} if out_parsing_errors is None else out_parsing_errors
        )
        ...
        self.module_to_resolved: dict[
            tuple[TFDefinitionKey | None, str], list[TFDefinitionKey]
        ] = {}
    ...
    def parse_directory(
        ...
        external_modules_content_cache: Optional[
            Dict[str, ModuleContent | None]
        ] = None,
    ...
    def parse_file(
        self, file: str, parsing_errors: dict[str, Exception]
    ...
    def _load_files(
        ...
        def _load_file(
            ...
            tuple[str, dict[str, list[dict[str, Any]]] | None], dict[str, Exception]
        ]:
        ...
        for file in list(self.out_definitions.keys()):
            ...
            for module_call in module_calls:
                ...
                for module_call_name, module_call_data in module_call.items():
                    ...
                    file_key = self.get_file_key_with_nested_data(
                        file, nested_modules_data
                    )
                    ...
                    skip_module = self.should_skip_a_module(
                        specified_vars, ignore_unresolved_params
                    )
                    if skip_module:
                    ...
                    source = self.get_module_source(
                        module_call_data, module_call_name, file
                    )
                    if not source:
                        continue
                    ...
                    try:
                        content_path = self.get_content_path(
                            module_loader_registry, root_dir, source, version
                        )
                        if not content_path:
                            continue
                        ...
                        module_definitions = {
                            ...
                            and not path.tf_source_modules
                        }
                        ...
                        for key in keys:
                            ...
                            new_key = self.get_new_nested_module_key(
                                key, file, module_call_name, nested_modules_data
                            )
                        ...
                        self.external_modules_source_map[(source, version)] = (
                            content_path
                        )
    ...
    def create_definition_by_dirs(
        self, tf_definitions: dict[TFDefinitionKey, dict[str, list[dict[str, Any]]]]
            ...
            if source_module is None:
                ...
                while source_module is not None:
                    if source_module.nested_tf_module is None:
                        ...
                        dirs_to_definitions[dir_path].append(
                            {tf_definition_key: tf_value}
                        )
    ...
    def _update_resolved_modules(self) -> None:
        ...
        for key, resolved_list in self.module_to_resolved.items():
            ...
            idx = self.get_idx_by_module_name(
                self.out_definitions[file_key]["module"], module_name
            )
            if idx is None:
                continue

            self.out_definitions[file_key]["module"][idx][module_name][
                RESOLVED_MODULE_ENTRY_NAME
            ] = resolved_list

    @staticmethod
    def get_idx_by_module_name(
        module_data_list: list[dict[str, Any]], module_name: str
    ) -> int | None:
    ...
    def parse_hcl_module_from_tf_definitions(
        self,
        tf_definitions: dict[str, dict[str, Any]]
        | dict[TFDefinitionKey, dict[str, Any]],
        source_dir: str,
        source: str,
    ) -> tuple[
        Module, dict[str, dict[str, Any]] | dict[TFDefinitionKey, dict[str, Any]]
    ]:
        ...
        for tf_def in copy_of_tf_definitions:
            for file_path, blocks in tf_def.items():
                for block_type in blocks:
                    try:
                        module.add_blocks(
                            block_type, blocks[block_type], file_path, source
                        )
                    except Exception as e:
                        logging.warning(
                            f"Failed to add block {blocks[block_type]}. Error:"
                        )
                        logging.warning(e, exc_info=False)
        return module, tf_definitions
    ...
    def get_file_key_with_nested_data(
        ...
        if not nested_data or file is None:
            return file
        nested_str = self.get_file_key_with_nested_data(
            nested_data.get("file"), nested_data.get("nested_modules_data")
        )
        nested_module_name = nested_data.get("module_name")
        return get_tf_definition_object_from_module_dependency(
            file, nested_str, nested_module_name
        )

    def get_new_nested_module_key(
        self,
        key: TFDefinitionKey,
        file: TFDefinitionKey | None,
        module_name: str | None,
        nested_data: Optional[dict[str, Any]],
    ) -> TFDefinitionKey:
        if not nested_data or not file:
            return get_tf_definition_object_from_module_dependency(
                key, file, module_name
            )
        visited_key_to_add = get_tf_definition_object_from_module_dependency(
            key, file, module_name
        )
        self.visited_definition_keys.add(visited_key_to_add)
        nested_key = self.get_new_nested_module_key(
            file,
    ...
    def add_tfvars_with_source_dir(
        self, module: Module, source: str, source_dir: str
    ) -> None:
        if not self.external_variables_data:
            return
        for var_name, default, path in self.external_variables_data:
    ...
    def get_module_source(
        self,
        module_call_data: dict[str, Any],
        module_call_name: str,
        file: TFDefinitionKey,
    ) -> Optional[str]:
        ...
        if source.startswith("./") or source.startswith("../"):
            ...
            source = os.path.normpath(
                os.path.join(
                    os.path.dirname(remove_module_dependency_from_path(file_to_load)),
                    source,
                )
            )
        return source
            ...
            for var_block in var_blocks:
                ...
                for var_name, var_definition in var_block.items():
                    ...
                    if default_value is not None and isinstance(default_value, list):
                        self.external_variables_data.append(
                            (var_name, default_value[0], file)
                        )
    ...
    def handle_variables(
        ...
        for file in dir_contents:
            ...
            elif file.name.endswith(".tf") or file.name.endswith(
                ".hcl"
            ):  # TODO: add support for .tf.json
        ...
        if hcl_tfvars:  # terraform.tfvars
            data = load_or_die_quietly(
                hcl_tfvars, self.out_parsing_errors, clean_definitions=False
            )
            if data:
                self.external_variables_data.extend(
                    [(k, safe_index(v, 0), hcl_tfvars.path) for k, v in data.items()]
                )
        ...
        for var_file, data in sorted(auto_var_files_to_data, key=lambda x: x[0]):
            if data:
                self.external_variables_data.extend(
                    [(k, v, var_file) for k, v in data.items()]
                )
        ...
        if vars_files:
            for var_file, data in sorted(
                explicit_var_files_to_data, key=lambda x: vars_files.index(x[0])
            ):
                if data:
                    self.external_variables_data.extend(
                        [(k, v, var_file) for k, v in data.items()]
    ...
    def is_valid_source(source: Any, module_call_name: str) -> TypeGuard[str]:
        if not isinstance(source, str):
            ...
            logging.debug(
                f"Skipping loading of {module_call_name} as source is the current dir"
            )
            return False
        return True

    @staticmethod
    def should_skip_a_module(
        specified_vars: dict[str, Any], ignore_unresolved_params: bool
    ) -> bool:
        if not ignore_unresolved_params:
            has_unresolved_params = False
            for k, v in specified_vars.items():
                if not is_acceptable_module_param(v) or not is_acceptable_module_param(
                    k
                ):
...
def get_tf_definition_object_from_module_dependency(
    path: TFDefinitionKey,
    module_dependency: TFDefinitionKey | None,
    module_dependency_name: str | None,
) -> TFDefinitionKey:
    ...
    if not is_nested_object(module_dependency):
        return TFDefinitionKey(
            path.file_path,
            TFModule(path=module_dependency.file_path, name=module_dependency_name),
        )
    return TFDefinitionKey(
        path.file_path,
        TFModule(
            path=module_dependency.file_path,
            name=module_dependency_name,
