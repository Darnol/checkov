...
from checkov.terraform.graph_builder.foreach.consts import (
    FOR_EACH_BLOCK_TYPE,
    FOREACH_STRING,
    COUNT_STRING,
)
...
class ForeachEntityHandler(ForeachAbstractHandler):
    def __init__(
        self,
        local_graph: TerraformLocalGraph,
        block_type_to_handle: str,
    ) -> None:
        super().__init__(local_graph)
        self.block_type_to_handle = block_type_to_handle

    def handle(self, resources_blocks: list[int]) -> None:
        block_index_to_statement: FOR_EACH_BLOCK_TYPE = self._get_statements(
            resources_blocks
        )
    ...
    def _get_statements(self, resources_blocks: list[int]) -> FOR_EACH_BLOCK_TYPE:
        ...
        if blocks_to_render:
            rendered_statements: FOR_EACH_BLOCK_TYPE = self._handle_dynamic_statement(
                blocks_to_render
            )
            block_index_to_statement.update(rendered_statements)
        return block_index_to_statement

    def _get_static_foreach_statement(
        self,
        block_index: int,
    ...
    def _handle_dynamic_statement(
        self,
        blocks_to_render: list[int],
    ) -> FOR_EACH_BLOCK_TYPE:
        rendered_statements_by_idx: FOR_EACH_BLOCK_TYPE = {}
        sub_graph = self._build_sub_graph(blocks_to_render)
        self._render_sub_graph(
            sub_graph,
            blocks_to_render,
        )
        for block_idx in blocks_to_render:
            if not self._is_static_statement(
                block_idx,
                sub_graph,
            ):
                rendered_statements_by_idx[block_idx] = None
            else:
                rendered_statements_by_idx[block_idx] = self._handle_static_statement(
                    block_idx,
                    sub_graph,
                )
        return rendered_statements_by_idx

    def _create_new_resources_count(
        self,
        statement: int,
        block_idx: int,
    ) -> None:
        ...
        for i in range(statement):
            append_virtual_resource(
                self._create_new_resource(
                    ...
                    new_value=i,
                    resource_idx=block_idx,
                    foreach_idx=i,
                ),
                virtual_resources_names,
            )
        if env_vars_config.RAW_TF_IN_GRAPH_ENV:
            main_resource.config[CustomAttributes.VIRTUAL_RESOURCES] = (
                virtual_resources_names
            )
            self.local_graph.vertices.append(main_resource)

    def _create_new_foreach_resource(
        self,
        block_idx: int,
        foreach_idx: int,
    ...
    def _create_new_resource(
        ...
        key_to_val_changes = self._build_key_to_val_changes(
            main_resource,
            new_value,
            new_key,
        )
        ...
        self._update_foreach_attrs(
            config_attrs,
            key_to_val_changes,
            new_resource,
        )
        idx_to_change = new_key or new_value
        self._add_index_to_resource_block_properties(
            new_resource,
            idx_to_change,
        )
        if foreach_idx == 0:
    ...
    @staticmethod
    def _add_index_to_resource_block_properties(
        block: TerraformBlock, idx: str | int
    ) -> None:
        block_type, block_name = block.name.split(".")
        idx_with_separator = ForeachEntityHandler._update_block_name_and_id(
            block,
            idx,
        )
        if block.config.get(block_type) and block.config.get(block_type, {}).get(
            block_name
        ):
            block.config[block_type][f"{block_name}[{idx_with_separator}]"] = (
                block.config[block_type].pop(block_name)
            )

    def _create_new_resources(
        self,
        block_index_to_statement: FOR_EACH_BLOCK_TYPE,
    ) -> None:
        for block_idx, statement in block_index_to_statement.items():
            ...
            if isinstance(statement, int):
                self._create_new_resources_count(
                    statement,
                    block_idx,
                )
            else:
                self._create_new_resources_foreach(
                    statement,
                    block_idx,
                )
