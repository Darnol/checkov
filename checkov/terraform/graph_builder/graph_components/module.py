...
from checkov.common.graph.graph_builder import (
    CustomAttributes,
    wrap_reserved_attributes,
    reserved_attributes_to_scan,
)
...
_AddBlockTypeCallable: TypeAlias = (
    "Callable[[Module, list[dict[str, dict[str, Any]]], TFDefinitionKeyType], None]"
)
...
class Module:
    def __init__(
        ...
        self.render_dynamic_blocks_env_var = os.getenv(
            "CHECKOV_RENDER_DYNAMIC_MODULES", "True"
        )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self.resources_types == other.resources_types
            and self.source_dir == other.source_dir
    ...
    def from_dict(module_dict: dict[str, Any]) -> Module:
        module = Module(
            source_dir=module_dict.get("source_dir", ""),
            external_modules_source_map=Module._from_dict_external_modules_source_map(
                module_dict
            ),
        )
        module.blocks = [
            TerraformBlock.from_dict(block) for block in module_dict.get("blocks", [])
        ]
        ...
        module.render_dynamic_blocks_env_var = module_dict.get(
            "render_dynamic_blocks_env_var", ""
        )
        return module

    def _to_dict_external_modules_source_map(self) -> dict[str, str]:
        return {
            str(k_tuple): v for k_tuple, v in self.external_modules_source_map.items()
        }

    @staticmethod
    def _from_dict_external_modules_source_map(
        module_dict: dict[str, Any],
    ) -> dict[tuple[str, str], Any]:
        return {
            literal_eval(k_tuple): v
            for k_tuple, v in module_dict.get("external_modules_source_map", {}).items()
        }
    ...
    def _add_provider(
        self, blocks: List[Dict[str, Dict[str, Any]]], path: TFDefinitionKeyType
    ) -> None:
    ...
    def _alter_reserved_attributes(self, attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for reserved_attribute in reserved_attributes_to_scan:
            if reserved_attribute in updated_attributes:
                updated_attributes[wrap_reserved_attributes(reserved_attribute)] = (
                    updated_attributes[reserved_attribute]
                )
            ...
            for resource_type, resources in resource_dict.items():
                ...
                for name, resource_conf in resources.items():
                    ...
                    if self.render_dynamic_blocks_env_var.lower() == "false":
                        ...
                        dynamic_attributes = {
                            k: attributes[k]
                            for k in set(attributes) - set(old_attributes)
                        }
    ...
    def clean_bad_characters(resource_conf: dict[str, Any]) -> dict[str, Any]:
        try:
            return cast(
                "dict[str, Any]",
                json.loads(json.dumps(resource_conf).replace("\\\\", "\\")),
            )
    ...
    @staticmethod
    def _handle_provisioner(
        provisioner: List[Dict[str, Any]], attributes: Dict[str, Any]
    ) -> None:
