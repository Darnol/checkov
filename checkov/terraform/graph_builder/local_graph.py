...
from checkov.common.graph.graph_builder.utils import (
    calculate_hash,
...
from checkov.terraform.graph_builder.foreach.utils import (
    get_terraform_foreach_or_count_key,
    get_sanitized_terraform_resource_id,
)
from checkov.terraform.graph_builder.utils import is_local_path
from checkov.terraform.graph_builder.variable_rendering.renderer import (
    TerraformVariableRenderer,
...
class TerraformLocalGraph(LocalGraph[TerraformBlock]):
    def __init__(self, module: Module) -> None:
        ...
        self.vertices_by_module_dependency_by_name: Dict[
            TFModule | None,
            Dict[
                str,
                Dict[str, List[int]],
        ...
        self.vertices_by_module_dependency: Dict[
            TFModule | None, Dict[str, List[int]]
        ] = defaultdict(partial(defaultdict, list))
        self.enable_foreach_handling = strtobool(
            os.getenv("CHECKOV_ENABLE_FOREACH_HANDLING", "True")
        )
        self.enable_modules_foreach_handling = strtobool(
            os.getenv("CHECKOV_ENABLE_MODULES_FOREACH_HANDLING", "True")
        )
        self.enable_datas_foreach_handling = strtobool(
            os.getenv("CHECKOV_ENABLE_DATAS_FOREACH_HANDLING", "False")
        )
        self.foreach_blocks: Dict[str, List[int]] = {
            BlockType.RESOURCE: [],
            BlockType.MODULE: [],
            BlockType.DATA: [],
        }
    ...
    def build_graph(self, render_variables: bool) -> None:
        ...
        if (self.enable_foreach_handling or self.enable_modules_foreach_handling) and (
            self.foreach_blocks[BlockType.RESOURCE]
            or self.foreach_blocks[BlockType.MODULE]
            or self.foreach_blocks[BlockType.DATA]
        ):
            try:
                ...
                self._build_edges()
                logging.info(
                    f"[TerraformLocalGraph] finished handling foreach values with {len(self.vertices)} vertices and {len(self.edges)} edges"
                )
        ...
        if render_variables:
            logging.info(
                f"Rendering variables, graph has {len(self.vertices)} vertices and {len(self.edges)} edges"
            )
            ...
            if strtobool(
                ...
                logging.info(
                    f"Found {len(self.edges) - edges_count} cross variable edges"
                )
            ...
            logging.info(
                f"Found {len(self.edges) - edges_count} S3 name references edges"
            )
        else:
            self.update_vertices_fields()
    ...
    def update_vertices_fields(self) -> None:
        ...
        try:
            ...
            logging.info(
                f"Failed to add provider attributes to resources, error: {str(e)}"
            )
    ...
    def _create_vertices(self) -> None:
        ...
        for i, block in enumerate(self.module.blocks):
            ...
            if (
                self.enable_foreach_handling
                and (
                    checkov.terraform.graph_builder.foreach.consts.FOREACH_STRING
                    in block.attributes
                    or checkov.terraform.graph_builder.foreach.consts.COUNT_STRING
                    in block.attributes
                )
                and block.block_type
                in (BlockType.MODULE, BlockType.RESOURCE, BlockType.DATA)
            ):
    ...
    def _add_block_data_to_graph(self, idx: int, block: TerraformBlock) -> None:
        ...
        self.vertices_by_module_dependency[block.source_module_object][
            block.block_type
        ].append(idx)
        self.vertices_by_module_dependency_by_name[block.source_module_object][
            block.block_type
        ][block.name].append(idx)
    ...
    def _add_provider_attr_to_resources(self) -> None:
        ...
        for vertex in self.vertices:
            if vertex.block_type == BlockType.RESOURCE:
                path_for_tf_definition = TFDefinitionKey(
                    file_path=vertex.path, tf_source_modules=vertex.source_module_object
                )
                if BlockType.PROVIDER in vertex.attributes:
                    provider = self._get_the_default_provider(
                        vertex.attributes[BlockType.PROVIDER], path_for_tf_definition
                    )
                    self._assign_provider_fields(vertex, provider)
                elif BlockType.PROVIDER in self.module.temp_tf_definition.get(
                    path_for_tf_definition, {}
                ):
                    provider_name = self._get_the_default_provider(
                        self.module.temp_tf_definition.get(
                            path_for_tf_definition, {}
                        ).get(BlockType.PROVIDER, []),
                        path_for_tf_definition,
                        is_same_file=True,
                    )
                    v_name = vertex.name.split(".")
                    provider_name = vertex.config[v_name[0]][v_name[1]][
                        CustomAttributes.TF_RESOURCE_ADDRESS
                    ].replace(vertex.name, provider_name)
                    self._assign_provider_fields(vertex, provider_name)
                else:
                    while path_for_tf_definition.tf_source_modules:
                        if (
                            self.module.temp_tf_definition
                            and path_for_tf_definition.tf_source_modules
                            and BlockType.PROVIDER
                            in self.module.temp_tf_definition.get(
                                path_for_tf_definition, ""
                            )
                        ):
                            module = [
                                m
                                for m in self.module.temp_tf_definition.get(
                                    path_for_tf_definition
                                ).get(BlockType.MODULE)
                                if list(m.keys())[0] == vertex.source_module_object.name
                            ]  # type:ignore
                            provider_name = self._get_the_default_provider(
                                self.module.temp_tf_definition.get(
                                    TFDefinitionKey(
                                        path_for_tf_definition.tf_source_modules.path
                                    ),
                                    {},
                                ).get(BlockType.PROVIDER, []),
                                path_for_tf_definition,
                            ...
                            path_for_tf_definition.tf_source_modules
                            and BlockType.PROVIDER
                            in self.module.temp_tf_definition.get(
                                TFDefinitionKey(
                                    path_for_tf_definition.tf_source_modules.path
                            ...
                            if (
                                vertex.source_module_object
                                and vertex.source_module_object.nested_tf_module
                            ):
                                module = [
                                    m
                                    for m in self.module.temp_tf_definition.get(
                                    ...
                                    if list(m.keys())[0]
                                    == vertex.source_module_object.nested_tf_module.name
                                ]  # type:ignore
                                provider_name = self._get_the_default_provider(
                                    self.module.temp_tf_definition.get(
                                        TFDefinitionKey(
                                ...
                                module = [
                                    ...
                                    if list(m.keys())[0]
                                    == vertex.source_module_object.name
                                ]  # type:ignore
                                provider_name = self._get_the_default_provider(
                                    self.module.temp_tf_definition.get(
                                        TFDefinitionKey(
                            ...
                            path_for_tf_definition = TFDefinitionKey(
                                tf_source_modules=path_for_tf_definition.tf_source_modules.nested_tf_module,
    ...
    def _assign_provider_fields(vertex: TerraformBlock, provider_name: str) -> None:
        ...
        v_name = vertex.name.split(".")
        vertex.config[v_name[0]][v_name[1]][CustomAttributes.PROVIDER_ADDRESS] = (
            provider_name
        )

    def _get_provider_address(
        self, path_for_tf_definition: TFDefinitionKeyType
    ) -> list[int] | None:
        if isinstance(path_for_tf_definition, TFDefinitionKey):
            return self.vertices_by_module_dependency[
                path_for_tf_definition.tf_source_modules
            ].get(BlockType.PROVIDER)
        return None
    ...
    def _get_the_default_provider(
        ...
        if module and len(module) > 0:
            module_providers = module[0][list(module[0].keys())[0]].get(
                "providers", [""]
            )[0]
            if module_providers:
                for _, m_alias in module_providers.items():
                    if not provider_address:
                        return cast(
                            str,
                            module_providers[list(module_providers.keys())[0]]
                            .replace(DOLLAR_PREFIX, "")
                            .replace(LEFT_CURLY, "")
                            .replace(RIGHT_CURLY, ""),
                        )
                    else:
                        for p_address in provider_address:
                            if (
                                m_alias.replace(DOLLAR_PREFIX, "")
                                .replace(LEFT_CURLY, "")
                                .replace(RIGHT_CURLY, "")
                                == self.vertices[p_address].name
                            ):
                                return cast(
                                    str,
                                    self.vertices[p_address]
                                    .config[list(self.vertices[p_address].config)[0]]
                                    .get(CustomAttributes.TF_RESOURCE_ADDRESS),
                                )
                    ...
                    for address in provider_address:
                        if self.vertices[address].name == provider:
                            return cast(
                                ...
                                .config[list(self.vertices[address].config)[0]]
                                .get(CustomAttributes.TF_RESOURCE_ADDRESS),
                            )
        else:
            for provider in providers:
                    ...
                    if provider_address and not is_same_file:
                        for p_address in provider_address:
                            if self.vertices[p_address].name == provider_name:
                                return cast(
                                    str,
                                    self.vertices[p_address].config[
                                        list(self.vertices[p_address].config.keys())[0]
                                    ][CustomAttributes.TF_RESOURCE_ADDRESS],
                                )
    ...
    def _arrange_graph_data(self) -> None:
        ...
        self.vertices_by_module_dependency_by_name = defaultdict(
        ...
        for vertex in self.vertices:
            ...
            for idx in self.vertices_by_block_type[BlockType.MODULE]:
                ...
                if vertex.source_module_object.path != self.vertices[idx].path:
                    continue
                if (
                    vertex.source_module_object.nested_tf_module
                    != self.vertices[idx].source_module_object
                ):
                    continue
                if (
                    vertex.source_module_object.foreach_idx
                    != self.vertices[idx].for_each_index
                ):
                    continue
    ...
    def _build_edges(self) -> None:
        ...
        for origin_node_index, vertex in enumerate(self.vertices):
            # DEBUG - Stop for first instance or subnet
            if any([term in vertex.id for term in ["instance"]]):
                pass

            self._build_edges_for_vertex(
                origin_node_index,
                vertex,
                aliases,
                resources_types,
            )
            self._build_virtual_resources_edges(
                origin_node_index,
                vertex,
            )

    def _build_edges_for_vertex(
        self,
        origin_node_index: int,
        vertex: TerraformBlock,
        ...
        for attribute_key, attribute_value in vertex.attributes.items():
            # DEBUG - Stop for interesting attributes
            if attribute_key in ["subnet_id"]:
                pass

            if (
                attribute_key in reserved_attribute_names
                or attribute_has_nested_attributes(
                    attribute_key,
                    vertex.attributes,
                    attribute_is_leaf,
                )
            ...
            for vertex_reference in referenced_vertices:
                ...
                # ]
                sub_values = vertex_reference.sub_parts
                for i in range(len(sub_values)):
                    ...
                    if referenced_modules is not None:
                        for module in referenced_modules:
                            ...
                            self._create_edge_from_reference(
                                attribute_key,
                                origin_node_index,
                                dest_node_index,
                                sub_values,
                    ...
                    if vertex.source_module_object:
                        ...
                        dest_node_index = self._find_vertex_index_relative_to_path(
                            ...
                            source_module_object=source_module_object,
                            origin_vertex_index=origin_node_index,
                        )
                    if dest_node_index > -1 and origin_node_index > -1:
                        self._create_edge_from_reference(
                            ...
                            vertex_reference=vertex_reference,
                            cross_variable_edges=cross_variable_edges,
                        )
                        break

        if (
            vertex.block_type == BlockType.MODULE
            and vertex.attributes.get("source")
            ...
            for attribute in vertex.attributes.keys():
                ...
                target_variable = next(
                    (v for v in target_variables if self.vertices[v].name == attribute),
                    None,
                )
            ...
            target_variable = 0
            for index in self.vertices_block_name_map.get(BlockType.VARIABLE, {}).get(
                vertex.name, []
            ):
                if self.get_dirname(self.vertices[index].path) == self.get_dirname(
                    vertex.path
                ):
            ...
            if target_variable:
                self.create_edge(
                    target_variable, origin_node_index, "default", cross_variable_edges
                )
    ...
    def _get_target_variables(
        self, vertex: TerraformBlock, dest_module_path: str
    ) -> list[int]:
        target_path = get_vertex_as_tf_module(vertex)
        return [
            index
            for index in self.vertices_by_module_dependency.get(target_path, {}).get(
                BlockType.VARIABLE, []
            )
        ...
        for origin_node_index, referenced_vertices in self.out_edges.items():
            ...
            if vertex.block_type == BlockType.MODULE:
                try:
                    ...
                    if tf_def and isinstance(tf_def, list):
                        ...
                        if resources:
                            ...
                            for e in referenced_vertices:
                                if (
                                    self.vertices[e.dest].block_type
                                    == BlockType.PROVIDER
                                ):
                    ...
                    logging.warning(
                        f"Failed in connecting module resources to provider due to {e}"
                    )
    ...
    def _build_cross_variable_edges(self) -> None:
        ...
        for origin_node_index, referenced_vertices in self.out_edges.items():
            vertex = self.vertices[origin_node_index]
            if vertex.block_type == BlockType.RESOURCE and any(
                self.vertices[e.dest].block_type != BlockType.RESOURCE
                for e in referenced_vertices
            ):
                modules = vertex.breadcrumbs.get(CustomAttributes.SOURCE_MODULE, [])
                self._build_edges_for_vertex(
                    origin_node_index, vertex, aliases, resources_types, True, modules
                )
    ...
    def _build_s3_name_reference_edges(self) -> None:
        ...
        for origin_node_index, referenced_vertices in self.out_edges.items():
            ...
            for referenced_vertice in referenced_vertices:
                if referenced_vertice.label == S3_BUCKET_REFERENCE_ATTRIBUTE:
                    current = s3_buckets_mapping.get(
                        referenced_vertice.dest,
                        {"bucket_resource_index": None, "referenced_vertices": list()},
                    )
        ...
        for destination, mapping in s3_buckets_mapping.items():
            if self.vertices[destination].block_type in [
                BlockType.VARIABLE,
                BlockType.LOCALS,
            ]:
                ...
                for reference_vertex in mapping["referenced_vertices"]:
                    self.create_edge(
                        mapping["bucket_resource_index"],
                        reference_vertex.origin,
                        S3_BUCKET_REFERENCE_ATTRIBUTE,
                        True,
                    )
    ...
    def create_edge(
        ...
        if cross_variable_edges:
            if (
                self.vertices[dest_vertex_index].block_type != BlockType.RESOURCE
                or self.vertices[origin_vertex_index].block_type != BlockType.RESOURCE
            ):
                return False
    ...
    def _connect_module(
        ...
        if len(sub_values) > 1:
            ...
            output_blocks_with_name = self.vertices_block_name_map.get(
                BlockType.OUTPUT, {}
            ).get(block_name_in_other_module, [])
            for vertex_index in output_blocks_with_name:
                vertex = self.vertices[vertex_index]
                if self._should_add_edge(vertex, dest_module_path, module_node):
                    added_edge = self.create_edge(
                        origin_node_index,
                        vertex_index,
                        attribute_key,
                        cross_variable_edges,
                    ...
                    if added_edge:
                        self.vertices[origin_node_index].add_module_connection(
                            attribute_key, vertex_index
                        )
                    break

    def _get_dest_module_path(
        self, curr_module_dir: str, dest_module_source: str, dest_module_version: str
    ) -> str:
        """
        ...
        if relative_path_key in self.relative_paths_cache:
            ...
            self.relative_paths_cache[relative_path_key] = os.path.abspath(
                Path(curr_module_dir) / dest_module_source
            )
            return self.relative_paths_cache[relative_path_key]
        elif (
            dest_module_source,
            dest_module_version,
        ) in self.module.external_modules_source_map:
    ...
    def _find_vertex_index_relative_to_path(
        ...
        possible_vertices = self._get_possible_vertices(
            module_dependency_by_name_key,
            block_type,
            name,
        )
        ...
        if len(relative_vertices) == 1:
            ...
            relative_vertex = self._find_vertex_with_best_match(
                relative_vertices, block_path, origin_vertex_index
            )
        return relative_vertex

    def _get_possible_vertices(
        self,
        module_dependency_by_name_key: TFModule | None,
        block_type: str,
        name: str,
    ) -> list[int]:
        possible_vertices = (
            self.vertices_by_module_dependency_by_name.get(
                module_dependency_by_name_key, {}
            )
            .get(block_type, {})
            .get(name, [])
        ...
        return (
            ...
            .get(block_type, {})
            .get(
                name.replace(LEFT_BRACKET_WITH_QUOTATION, LEFT_BRACKET).replace(
                    RIGHT_BRACKET_WITH_QUOTATION, RIGHT_BRACKET
                ),
                [],
            )
        )

    def _find_vertex_with_best_match(
        self,
        relevant_vertices_indexes: List[int],
        origin_path: str,
        origin_vertex_index: Optional[int] = None,
    ) -> int:
        ...
        for vertex_index in relevant_vertices_indexes:
            ...
            if origin_vertex_index is not None:
                vertex_module_name = vertex.attributes.get(
                    CustomAttributes.TF_RESOURCE_ADDRESS, ""
                )
                origin_module_name = self.vertices[origin_vertex_index].attributes.get(
                    CustomAttributes.TF_RESOURCE_ADDRESS, ""
                )
                if vertex_module_name.startswith(
                    BlockType.MODULE
                ) and origin_module_name.startswith(BlockType.MODULE):
                    split_module_name = vertex_module_name.split(".")[1]
                    if origin_module_name.startswith(
                        f"{BlockType.MODULE}.{split_module_name}"
                    ):
                        common_prefix = (
                            f"{common_prefix} {BlockType.MODULE}.{split_module_name}"
                        )
        ...
        if origin_vertex_index is not None:
            return self._find_best_match_based_on_foreach_key(
                origin_vertex_index,
                vertices_with_longest_common_prefix,
    ...
    def _find_best_match_based_on_foreach_key(
        ...
        for vertex_index, vertex in vertices_with_longest_common_prefix:
            vertex_address = vertex.attributes.get(
                CustomAttributes.TF_RESOURCE_ADDRESS, ""
            )
            vertex_foreach_value = vertex.for_each_index
            origin_address = origin_vertex.attributes.get(
                CustomAttributes.TF_RESOURCE_ADDRESS, ""
            )
            origin_foreach_value = origin_vertex.for_each_index
            if (
                origin_foreach_value == vertex_foreach_value
                and origin_address != ""
                and get_terraform_foreach_or_count_key(origin_address)
    ...
    def get_vertices_hash_codes_to_attributes_map(self) -> Dict[str, Dict[str, Any]]:
        return {
            vertex.get_hash(): vertex.get_attribute_dict() for vertex in self.vertices
        }
    ...
    def order_edges_by_hash_codes(self) -> Dict[str, Edge]:
        ...
        for edge in self.edges:
            edge_data = {
                ...
                "from_vertex_hash": self.get_vertex_hash_by_index(
    ...
    def get_vertex_hash_by_index(self, vertex_index: int) -> str:
        return self.vertex_hash_cache.setdefault(
            vertex_index, self.vertices[vertex_index].get_hash()
        )
    ...
    def update_vertex_attribute(
        ...
        attribute_at_dest = self.vertices[change_origin_id].find_attribute(
            attribute_at_dest
        )
        if attribute_at_dest:
            previous_breadcrumbs = self.vertices[
                change_origin_id
            ].changed_attributes.get(attribute_at_dest, [])
    ...
    def update_vertex_config(
        vertex: TerraformBlock,
        changed_attributes: Union[List[str], Dict[str, Any]],
        ...
        for changed_attribute in changed_attributes:
            ...
            if new_value is not None:
                ...
                updated_config = update_dictionary_attribute(
                    updated_config, changed_attribute, new_value, dynamic_blocks
                )
        ...
        if len(changed_attributes) > 0:
            if vertex.block_type == BlockType.LOCALS:
                ...
                update_dictionary_attribute(
                    vertex.config, vertex_name, updated_local_config, dynamic_blocks
                )
                return

            update_dictionary_attribute(
                vertex.config, vertex_name, updated_config, dynamic_blocks
            )
        ...
        for vertex in self.vertices:
            for attribute_key, breadcrumbs_list in vertex.changed_attributes.items():
                ...
                for breadcrumb in breadcrumbs_list:
                    ...
                    breadcrumb["module_connection"] = (
                        self._determine_if_module_connection(breadcrumbs_list, v)
                    )
    ...
    @staticmethod
    def _determine_if_module_connection(
        breadcrumbs_list: List[int], vertex_in_breadcrumbs: TerraformBlock
    ) -> bool:
        """
    ...
    def _update_nested_modules_address(self) -> None:
        for vertex in self.vertices:
            ...
            resource_id = (
                ".".join(entity_context_path) if entity_context_path else vertex.name
            )
    ...
    def _should_add_edge(
        self, vertex: TerraformBlock, dest_module_path: str, module_node: TerraformBlock
    ) -> bool:
        if not vertex.source_module_object:
            return False
        ...
        return (self.get_dirname(vertex.path) == dest_module_path) and (
            vertex.source_module_object
            == module_node.source_module_object  # The vertex is in the same file
    ...
    def _build_virtual_resources_edges(
        self, origin_node_index: int, vertex: TerraformBlock
    ) -> None:
...
def to_list(data: Any) -> list[Any] | dict[str, Any]:
    if (
        ...
        and len(data) == 1
        and (isinstance(data[0], str) or isinstance(data[0], int))
    ):
        return data
...
def update_list_attribute(
    ...
    for i, config_value in enumerate(config):
        if entry_to_update == -1:
            config[i] = update_dictionary_attribute(
                config=config_value,
                key_to_update=key_to_update,
                new_value=new_value,
                dynamic_blocks=dynamic_blocks,
            ...
            config[i] = update_dictionary_attribute(
                config=config_value,
                key_to_update=".".join(key_parts[1:]),
                new_value=new_value,
                dynamic_blocks=dynamic_blocks,
            )
...
def get_vertex_as_tf_module(block: TerraformBlock) -> TFModule:
    ...
    return TFModule(
        path=block.path,
        name=block_name,
        nested_tf_module=block.source_module_object,
        foreach_idx=block.for_each_index,
    )
