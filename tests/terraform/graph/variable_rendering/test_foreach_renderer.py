...
from checkov.terraform.graph_builder.foreach.resource_handler import (
    ForeachResourceHandler,
)
...
def build_and_get_graph_by_path(path, render_var=False):
    ...
    local_graph, tf_definitions = graph_manager.build_graph_from_source_directory(
        resources_dir, render_variables=render_var
    )
...
def test_new_resources_count():
    ...
    for i, resource in enumerate(
        [local_graph.vertices[1], local_graph.vertices[6], local_graph.vertices[7]]
    ):
...
def test_new_resources_foreach():
    ...
    for resource in [
        local_graph.vertices[0],
        local_graph.vertices[1],
        local_graph.vertices[5],
        local_graph.vertices[6],
    ]:
        assert resource.name.endswith('["bucket_a"]') or resource.name.endswith(
            '["bucket_b"]'
        )
        assert resource.id.endswith('["bucket_a"]') or resource.id.endswith(
            '["bucket_b"]'
        )
        config_name = list(resource.config["aws_s3_bucket"].keys())[0]
        assert config_name.endswith('["bucket_a"]') or config_name.endswith(
            '["bucket_b"]'
        )
...
def test_resources_flow():
    ...
    assert local_graph.vertices_block_name_map["variable"] == {
        "foreach_map": [1],
        "test": [2],
    }
    assert local_graph.vertices_block_name_map["resource"] == {
        'aws_s3_bucket.foreach_map["bucket_a"]': [0],
    ...
    assert (
        resources[0]
        .config.get("aws_s3_bucket")
        .get('foreach_map["bucket_a"]')
        .get("__address__")
        == resource_a_name
    )
    ...
    assert (
        list(resources[0].config.get("aws_s3_bucket").keys())[0]
...
def test_tf_definitions_and_breadcrumbs():
    ...
    dir_name = "foreach_examples/depend_resources"
    local_graph, _ = build_and_get_graph_by_path(dir_name, render_var=True)
    tf_definitions, breadcrumbs = convert_graph_vertices_to_tf_definitions(
        local_graph.vertices, dir_name
    )
    ...
    resource_vertices = [
        vertex for vertex in local_graph.vertices if vertex.block_type == "resource"
    ]
    ...
    for name in ['["bucket_a"]', '["bucket_b"]']:
        assert (
            f"aws_s3_bucket.foreach_map{name}"
            in breadcrumbs[list(breadcrumbs.keys())[0]]
        )

        location_var = "location"
        assert list(
            breadcrumbs[list(breadcrumbs.keys())[0]][
                f"aws_s3_bucket.foreach_map{name}"
            ].keys()
        ) == [location_var]
        assert list(
        ...
        assert breadcrumbs[list(breadcrumbs.keys())[0]][
            f"aws_s3_bucket.foreach_map{name}"
        ][location_var][0]["path"].endswith("depend_resources/variable.tf")
        assert expected_breadcrumbs[list(expected_breadcrumbs.keys())[0]][
            f"aws_s3_bucket.foreach_map{name}"
        ][location_var][0]["path"].endswith("depend_resources/variable.tf")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"test_key": ["${test_val} ${test_val}"]},
            {"test_val": "new_val"},
            {"test_key": ["new_val new_val"]},
            ["test_key"],
        ),
        (
            {"test_key": {"nested_key": ["${test_val}"]}},
            {"test_val": "new_val"},
            {"test_key": {"nested_key": ["new_val"]}},
            ["test_key.nested_key"],
        ),
        (
            {"test_key": ["${test_val} test_val"]},
        ...
        (
            {"test_key": {"a": {"b": "${test_val}"}}},
            {"test_val": "new_val"},
            {"test_key": {"a": {"b": "new_val"}}},
            ["test_key.a.b"],
        ),
        (
        ...
        (
            ...
            {
                "each.value": {"name": "security", "product_owner": "barak@gmail.com"},
                "each.key": "security",
            },
            {
                "tags": [
...
def test_new_tf_parser_with_foreach_modules(checkov_source_path):
    ...
    local_graph, _ = build_and_get_graph_by_path(dir_name, render_var=True)
    tf_definitions, _ = convert_graph_vertices_to_tf_definitions(
        vertices=local_graph.vertices, root_folder=dir_name
    )

    assert len(tf_definitions.keys()) == 14
    assert (
        len([block for block in local_graph.vertices if block.block_type == "resource"])
        == 8
    )
    assert (
        len([block for block in local_graph.vertices if block.block_type == "module"])
        == 12
    )
    ...
    assert (
        first_module_vertex.name == 's3_module["a"]'
        and first_module_vertex.for_each_index == "a"
    )

    second_module_vertex = local_graph.vertices[1]
    assert (
        second_module_vertex.name == "s3_module2[0]"
        and second_module_vertex.for_each_index == 0
    )

    twenty_fifth_module_vertex = local_graph.vertices[25]
    assert (
        twenty_fifth_module_vertex.name == 's3_module["b"]'
        and twenty_fifth_module_vertex.for_each_index == "b"
    )

    thrirty_six_module_vertex = local_graph.vertices[36]
    assert (
        thrirty_six_module_vertex.name == "s3_module2[1]"
        and thrirty_six_module_vertex.for_each_index == 1
    )
    ...
    assert (
        len(
            tf_definitions[
                first_nested_module["module"][0]["inner_s3_module"]["__resolved__"][0]
            ]["resource"]
        )
        == 1
    )
    assert (
        len(
            tf_definitions[
                first_nested_module["module"][1]["inner_s3_module2"]["__resolved__"][0]
            ]["resource"]
        )
        == 1
    )
    assert (
        len(
            tf_definitions[
                second_nested_module["module"][0]["inner_s3_module"]["__resolved__"][0]
            ]["resource"]
        )
        == 1
    )
    assert (
        len(
            tf_definitions[
                second_nested_module["module"][1]["inner_s3_module2"]["__resolved__"][0]
            ]["resource"]
        )
        == 1
    )
    assert (
        len(
            tf_definitions[
                third_nested_module["module"][0]["inner_s3_module"]["__resolved__"][0]
            ]["resource"]
        )
        == 1
    )
    assert (
        len(
            tf_definitions[
                third_nested_module["module"][1]["inner_s3_module2"]["__resolved__"][0]
            ]["resource"]
        )
        == 1
    )
    assert (
        len(
            tf_definitions[
                fourth_nested_module["module"][0]["inner_s3_module"]["__resolved__"][0]
            ]["resource"]
        )
        == 1
    )
    assert (
        len(
            tf_definitions[
                fourth_nested_module["module"][1]["inner_s3_module2"]["__resolved__"][0]
            ]["resource"]
        )
        == 1
    )

    assert first_tf_module.file_path == os.path.join(
        checkov_source_path,
...
def test_tf_definitions_for_foreach_on_modules(checkov_source_path):
    ...
    for dir_name, definitions_path in dir_name_and_definitions_path:
        ...
        tf_definitions_after_handling_checkov_source = json.loads(
            tf_definitions_json, object_hook=object_hook
        )
...
def test_foreach_module_in_second_level_module(checkov_source_path):
    ...
    tf_definitions, _ = convert_graph_vertices_to_tf_definitions(
        vertices=graph.vertices, root_folder=dir_name
    )

    assert (
        len([block for block in graph.vertices if block.block_type == "module"]) == 10
    )
    assert (
        len([block for block in graph.vertices if block.block_type == "resource"]) == 8
    )
...
def test_foreach_module_and_resource(checkov_source_path):
    ...
    assert (
        len([block for block in graph.vertices if block.block_type == "resource"]) == 4
    )
    assert len(tf_definitions.keys()) == 3

    assert (
        graph.vertices[2].config["aws_s3_bucket_public_access_block"][
            'var_bucket["a"]'
        ]["__address__"]
        == 'module.s3_module["a"].aws_s3_bucket_public_access_block.var_bucket["a"]'
    )
    assert (
        graph.vertices[6].config["aws_s3_bucket_public_access_block"][
            'var_bucket["a"]'
        ]["__address__"]
        == 'module.s3_module["b"].aws_s3_bucket_public_access_block.var_bucket["a"]'
    )
    assert (
        graph.vertices[8].config["aws_s3_bucket_public_access_block"][
            'var_bucket["b"]'
        ]["__address__"]
        == 'module.s3_module["a"].aws_s3_bucket_public_access_block.var_bucket["b"]'
    )
    assert (
        graph.vertices[9].config["aws_s3_bucket_public_access_block"][
            'var_bucket["b"]'
        ]["__address__"]
        == 'module.s3_module["b"].aws_s3_bucket_public_access_block.var_bucket["b"]'
    )
...
@mock.patch.dict(
    os.environ,
    {
        ...
        "CHECKOV_ENABLE_DATAS_FOREACH_HANDLING": "True",
    },
)
def test_foreach_data(checkov_source_path):
    ...
    data_vertices_names = [
        block.name for block in graph.vertices if block.block_type == "data"
    ]
...
def test_foreach_data_with_resource(checkov_source_path):
    ...
    assert (
        graph.vertices[0].attributes["bucket"] == graph.vertices[3].attributes["bucket"]
    )
    assert (
        graph.vertices[1].attributes["bucket"] == graph.vertices[4].attributes["bucket"]
    )
    assert (
        graph.vertices[8].attributes["bucket"]
        == graph.vertices[10].attributes["bucket"]
    )
    assert (
        graph.vertices[9].attributes["bucket"]
        == graph.vertices[11].attributes["bucket"]
    )
...
def test_foreach_module_with_more_than_two_resources(checkov_source_path):
    ...
    assert (
        len([block for block in graph.vertices if block.block_type == "module"]) == 16
    )
    assert (
        len([block for block in graph.vertices if block.block_type == "resource"]) == 14
    )
...
def test__get_tf_module_with_no_foreach():
    module = TFModule(
        ...
        path="1",
        foreach_idx="1",
        nested_tf_module=TFModule(
            name="2", path="2", foreach_idx="2", nested_tf_module=None
        ),
    )
    result = ForeachModuleHandler._get_tf_module_with_no_foreach(module)
    assert result == TFModule(
        name="1",
        path="1",
        foreach_idx=None,
        nested_tf_module=TFModule(
            name="2", path="2", foreach_idx=None, nested_tf_module=None
        ),
    )
...
def test__get_module_with_only_relevant_foreach_idx():
    module = TFModule(
        ...
        nested_tf_module=TFModule(
            ...
            foreach_idx="2",
            nested_tf_module=TFModule(
                name="3", path="3", foreach_idx="3", nested_tf_module=None
            ),
        ),
    )
    original_key = TFModule(
        name="2",
        path="2",
        foreach_idx="2",
        nested_tf_module=TFModule(
    ...
    result = ForeachModuleHandler._get_module_with_only_relevant_foreach_idx(
        "test", original_key, module
    )
    assert result == TFModule(
        name="1",
        path="1",
...
def test_nested_foreach_with_variable_reference():
    ...
    graph_resources_filter = filter(
        lambda blk: blk.block_type == "resource", graph.vertices
    )
    graph_resources_created = list(
        map(lambda rsrc: rsrc.attributes["__address__"], graph_resources_filter)
    )

    assert len(graph_resources_created) is (
        resources_by_group_local_var * resources_by_files_local_var
    )
    assert graph_resources_created == [
        'module.files["blue"].aws_s3_bucket_object.this_file["test1"]',
...
def test_double_nested_foreach_with_variable_reference():
    ...
    graph_modules_filter = filter(
        lambda blk: blk.block_type == "module", graph.vertices
    )
    graph_modules_created = list(
        map(lambda rsrc: rsrc.attributes["__address__"], graph_modules_filter)
    )
    ...
    assert (
        len(graph_modules_created) is 6
    )  # 2 level1 modules, each has 2 level2 modules (total of 2 + 2*2 = 6)
    assert (
        len(graph_resources_created) is 8
    )  # 4 level2 modules, each has 2 resources (total of 2*2*2 = 8)
...
def test_foreach_renderer_with_raw_asset():
    ...
    for resource in [local_graph.vertices[6], local_graph.vertices[8]]:
        ...
        for virtual_resource in resource.config[CustomAttributes.VIRTUAL_RESOURCES]:
            assert virtual_resource.endswith(
                '["bucket_a"]'
            ) or virtual_resource.endswith('["bucket_b"]')
...
def test_count_resource_dependency(checkov_source_path):
    dir_name = "data_with_resource_count"
    graph, _ = build_and_get_graph_by_path(dir_name, render_var=True)

    assert len(graph.vertices) == 5
    assert len(graph.edges) == 4

    vertices_index_to_id = {i: v.id for i, v in enumerate(graph.vertices)}

    # Assert vertices are present
    vertices_ids = list(vertices_index_to_id.values())
    assert "aws_vpc.vpc" in vertices_ids
    assert "aws_subnet.subnet[0]" in vertices_ids
    assert "aws_subnet.subnet[1]" in vertices_ids
    assert "aws_instance.instance_a" in vertices_ids
    assert "aws_instance.instance_b" in vertices_ids

    # Convert vertex index to id
    data_edges = [
        {
            "origin": vertices_index_to_id[e.to_dict()["origin"]],
            "dest": vertices_index_to_id[e.to_dict()["dest"]],
        }
        for e in graph.edges
    ]

    assert {"origin": "aws_subnet.subnet[0]", "dest": "aws_vpc.vpc"} in data_edges
    assert {"origin": "aws_subnet.subnet[1]", "dest": "aws_vpc.vpc"} in data_edges
    assert {
        "origin": "aws_instance.instance_a",
        "dest": "aws_subnet.subnet[0]",
    } in data_edges
    assert {
        "origin": "aws_instance.instance_b",
        "dest": "aws_subnet.subnet[1]",
    } in data_edges
